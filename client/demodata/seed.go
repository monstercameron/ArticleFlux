...
var seedItems = []seedItem{
	// --- Quiet Systems ---------------------------------------------------------
	{
		...
		body: []string{
			...
			"<p>I want to be careful about what I am claiming, because the version of this argument that gets repeated is always the cartoon one. I am not saying databases are fast. I am saying that on this system, on this path, after four years of people optimising the thing they believed was slow, the remaining time had moved somewhere else and nobody had gone looking for it.</p>",
			...
			"<p>The middleware is the part worth dwelling on. It was forty lines. It had a test. It had been reviewed by two people, both of whom I respect, and neither of whom had any way of knowing that the function it called to add those two fields would round-trip a fourteen-kilobyte payload through a generic map. That fact is not in the function's name, its signature, its documentation, or its test. It is in its implementation, which is exactly where nobody looks when they are reviewing a caller.</p>",
			...
			"<h2>How it stayed hidden</h2>",
			"<p>Three things kept it invisible for four years, and I think all three are common enough to be worth naming.</p>",
			"<p><strong>The dashboard measured the database.</strong> Not because anyone decided the database was the only thing worth measuring, but because the database client came with instrumentation and the rest of the code did not. Every graph on the wall was a graph of query time, so every conversation about performance was a conversation about queries. The instrument decided the subject.</p>",
			"<p><strong>The profile was taken in the wrong place.</strong> There was a profiler, and it was pointed at the worker pool, where the interesting concurrency lived. The request path was considered boring. It was: it was also 89% of the latency.</p>",
			"<p><strong>Everyone had a story.</strong> The outage in the second year really was a query — a missing index on a table that had grown past the point where a sequential scan was survivable. That story was true, it was memorable, and it did what true memorable stories do, which is to stand in for a whole category of problem for years afterwards. Anybody proposing to look somewhere else was, implicitly, proposing that the story was wrong.</p>",
			"<h2>What we changed</h2>",
			"<p>Almost nothing, in the end, which is the part I find hardest to write up honestly. We deleted the middleware and moved its two fields into the handler that produced the object. We removed one of the two conversions. The third one turned out to be load-bearing for a client that has since been retired, so it went too, six weeks later, after somebody checked.</p>",
			"<p>Median latency fell by a bit under half. There was no clever change, no new dependency, and nothing anybody could reasonably write a conference talk about. The work was not the fix; the work was the six months of measurement that made the fix obvious enough that nobody argued about it.</p>",
			"<h2>The part that generalises, if any of it does</h2>",
			"<p>I am not going to pretend this turns into a rule about databases. The point is narrower and, I think, more useful: the subsystem your team talks about most is the subsystem you have already optimised. Attention is the thing that gets spent, and it gets spent where the graphs are. The time is somewhere nobody is looking, because if anybody were looking it would not still be there.</p>",
			"<p>So the practical suggestion is not \"profile more\", which everybody already agrees with and nobody does. It is: <em>instrument the boring part first</em>. The part with no graphs, no story, and no outage attached to it. That is where four years of nobody looking has been accumulating, and it is the only place a system this old can still have half its latency sitting in plain sight.</p>",
			"<p>One more thing, because somebody will ask. Yes, we could have found this in an afternoon with a flame graph, and I have had that pointed out to me several times since. It is true and it is not the lesson. Somebody had a flame graph in year two. It showed the encoder near the top, which everyone interpreted as \"encoding is expensive\", which is a true statement that leads nowhere. The graph was never wrong; the reading of it was, and it was wrong in the direction of the story everybody already believed.</p>",
			"<p>What made the difference in the end was not a better tool. It was writing down, before looking at anything, what each of us expected the breakdown to be — and then discovering that our four guesses summed to about 160% of the actual request time. That disagreement was the finding. Everything after it was just arithmetic.</p>",
		},
	},
	{
	...
	// --- The Long Table --------------------------------------------------------
	{
		...
		body: []string{
			...
			"<h2>The three rules, and only one of them matters</h2>",
			"<p>Cold water to start, so the proteins come out slowly and clarify rather than seize. No boil, ever — a rolling boil emulsifies the fat and you get something cloudy that tastes flat no matter how long you leave it. And no salt, because you do not yet know what this is going to become.</p>",
			"<p>Of those three, the second is the only one I would defend to somebody in a hurry. The first is a small improvement you could skip. The third is a habit rather than a rule; plenty of good cooks salt lightly at the start and correct later. But the boil is not negotiable, and it is not negotiable in a way you can see: two pots, same bones, same eight hours, one held at a bare shiver and one allowed to bubble, and they are different colours.</p>",
			"<h2>Six hours in</h2>",
			...
			"<p>I have tried and failed to find a reliable marker for when it happens. It is not a temperature, because the temperature has not changed in five hours. It is not volume, though the volume has come down. Ask three people to say when the pot turned and you will get three answers inside a forty-minute window, which is either evidence that the transition is gradual and the perception of it is sudden, or evidence that we are all fooling ourselves in a correlated way. I lean towards the first, without much to support it.</p>",
			...
			"<p>That is a strange thing to claim about a cooking technique, and I want to be honest that it is a claim about the cook rather than about the stock. The stock is fine either way. What is different is what you learned about waiting, and whether you will now recognise the same shape somewhere else — in bread, in a braise, in the fortnight a sourdough starter takes to stop being a science experiment and start being an ingredient.</p>",
			"<p>Every one of those has the same structure: a long stretch where nothing you do makes it go faster, followed by a change of state you did not cause. Most of what people call patience in a kitchen is actually the accumulated experience of having been through that structure enough times to trust it. You cannot be told it. You can only have stood next to the pot.</p>",
			"<h2>The practical bit, briefly</h2>",
			"<p>Bones from the freezer, whatever has accumulated. Cover by two fingers. Bring it to the point where a bubble breaks the surface every few seconds and hold it there — on most stoves that is the lowest setting with the lid a thumb's width off, and if your stove will not go that low, the oven at 90°C will. Skim once in the first hour and then leave it alone.</p>",
			"<p>Six hours for chicken, eight to ten for beef, and stop when it tastes of itself rather than when a number says to. Strain it, cool it fast in a shallow pan, and lift the fat off the next day. It keeps four days in the fridge and forever in the freezer, and it will make the next four things you cook better in a way nobody eating them will be able to name.</p>",
			"<p>Two things I got wrong for years, in case they are the same two. I used to add vegetables at the start, which is what every recipe I learned from says, and which means eight hours of carrot — by the end it has given everything it has and then started taking things back. Now they go in for the last ninety minutes and the difference is not subtle. And I used to strain through a fine sieve while it was still hot, which drags fine sediment through with the pressure of the liquid above it. Let it settle off the heat for twenty minutes and ladle from the top instead; what is left in the bottom of the pot is the cloudiness you were trying to sieve out.</p>",
			"<p>Neither of those is a discovery. Both are in books I owned at the time. I mention them because the gap between reading an instruction and understanding what it is protecting you from is usually a few years wide, and the only thing that closes it is having made the worse version enough times to recognise the taste.</p>",
		},
	},
	{
