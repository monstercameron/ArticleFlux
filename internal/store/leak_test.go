...
var unscopedByDesign = map[string]string{
	...
	"RevokeSessionAndFamily":  "keyed by token hash, exactly like RevokeSession — the token IS the authorisation, and it names its own family",
	"FamilyForSession":        "keyed by token hash, like SessionAuthenticatedAt: it resolves a family from a session the caller has already proved it holds",
	"ChangePasswordAndRevoke": "break-glass reset and the sudo-gated RPC both call this by user id; the CLI has no session (like SetPasswordHash), and the RPC's authorisation is the fresh sudo window requireSudo already checked",
	...
	"UserForRecovery":         "produces the identity a Scope is built from, like UserForLogin, for a credential that NAMES its user instead of being presented by one: a reset token carries a user id and nothing else (§7.2). Returns no password hash — nothing on that path verifies one — and refuses a deactivated account.",
	"AuditTrailInstance":      "the INSTANCE view of the audit log, for the operator CLI (§7.3c). Some security events are recorded before an account is resolved — a login lockout above all, where the username may name nothing — so those rows carry no tenant and the scoped AuditTrail cannot return them. Relaxing THAT query to include NULLs would hand one tenant's admin another tenant's failed-login usernames, so the instance view is a separate method with a narrower audience: somebody holding the database file.",
	...
	"PurgeAuditLog": "housekeeping over the audit log by age alone. Unscoped for the same reason " +
		"AuditTrailInstance is: instance-level rows carry no tenant, and a scoped purge would leave " +
		"exactly those behind forever.",
