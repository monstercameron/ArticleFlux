...
const (
	...
	ReaderService_CountUnreadByCategory_FullMethodName  = "/articleflux.v1.ReaderService/CountUnreadByCategory"
...
type ReaderServiceClient interface {
	...
	//
	// Its own RPC rather than a field on ListFeeds, which the rail also calls:
	// this costs a few hundred milliseconds against a real database and the
	// sidebar's feeds must not wait behind it. The client asks separately and
	// fills the numbers in when they land.
	CountUnreadByCategory(ctx context.Context, in *CountUnreadByCategoryRequest, opts ...grpc.CallOption) (*CountUnreadByCategoryResponse, error)
...
func (c *readerServiceClient) CountUnreadByCategory(ctx context.Context, in *CountUnreadByCategoryRequest, opts ...grpc.CallOption) (*CountUnreadByCategoryResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(CountUnreadByCategoryResponse)
	err := c.cc.Invoke(ctx, ReaderService_CountUnreadByCategory_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type ReaderServiceServer interface {
	...
	// sidebar's feeds must not wait behind it. The client asks separately and
	// fills the numbers in when they land.
	CountUnreadByCategory(context.Context, *CountUnreadByCategoryRequest) (*CountUnreadByCategoryResponse, error)
	// Subscribe adds a feed by URL. The source is global (A14), so this usually
	// attaches an existing row rather than creating one.
...
func (UnimplementedReaderServiceServer) CountUnreadByCategory(context.Context, *CountUnreadByCategoryRequest) (*CountUnreadByCategoryResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method CountUnreadByCategory not implemented")
}
...
func (UnimplementedReaderServiceServer) AcceptRecommendation(context.Context, *AcceptRecommendationRequest) (*AcceptRecommendationResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method AcceptRecommendation not implemented")
}
...
func _ReaderService_CountUnreadByCategory_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(CountUnreadByCategoryRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(ReaderServiceServer).CountUnreadByCategory(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: ReaderService_CountUnreadByCategory_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(ReaderServiceServer).CountUnreadByCategory(ctx, req.(*CountUnreadByCategoryRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var ReaderService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "CountUnreadByCategory",
			Handler:    _ReaderService_CountUnreadByCategory_Handler,
		},
		{
