...
function Invoke-Wasm {
    ...
    # The homepage's screenshots (client/view/home.go). Pictures of the running
    # application, regenerated by `node e2e/home-shots.mjs` when the reader's
    # chrome changes. Like /audio/ they are not part of the app shell: nothing in
    # the reader references them, and a deployment that omits them loses the
    # front door's illustrations rather than the reader.
    $shotsSrc = Join-Path $WebSrc 'shots'
    if (Test-Path $shotsSrc) {
        $shotsOut = Join-Path $OutDir 'shots'
        if (Test-Path $shotsOut) { Remove-Item $shotsOut -Recurse -Force }
        Copy-Item $shotsSrc $shotsOut -Recurse -Force
    }
