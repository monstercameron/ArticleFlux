...
|---|---|
| **Status** | ✅ |
| **Spec** | `client/view/home.go`, `client/design/home.go`, `client/i18n/en_home.go` |

**`/home` is the page that explains the product**, to somebody who has not signed in and to anybody
deciding whether to run it. Eight sections — why it exists, reading, finding it again, what it
learns, reaching the page, listening, what leaves the box, how it is built — each illustrated with a
screenshot of the running reader rather than a mock-up (`web/shots/`, regenerated by
`node e2e/home-shots.mjs`).

**It is a component, not an HTML file, and that is load-bearing three times over:**

- **A26 applies to it.** There is no marketing page exempt from "the stylesheet is Go". The first
  version was a hand-written `web/home.html` with its own copy of the palette, and it was wrong.
- **The server's CSP caught it before anyone argued.** `script-src` hashes the SHELL's inline boot
  script, so the static page's inline script was blocked on the first load. The rules this
  application is built under do not stop at the reader's door.
- **It paints from `client/design`'s tokens**, so it follows the visitor's theme, accent, reading
  size and motion setting. A reader who set Daylight gets a Daylight homepage — which a separate
  stylesheet could not have done.

**Root decides it from the ADDRESS, before WhoAmI** (`phaseHome`, `isHomePath`). A visitor with no
account must not wait on a round trip that can only answer "nobody" before they are allowed to read
what this is. `/home` deliberately does not appear in `route.go`'s grammar: that grammar describes
places inside the reader, and this is not one.

**The page obeys the reader's key map.** `j` and `k` move between sections, `o` opens the demo, `?`
shows the sheet — and the sheet is the reader's own, resolved from the `help` namespace rather than a
second copy of it. The claim being made is that muscle memory transfers on day one, and a page that
asserts that while ignoring `j` argues against itself. The rail is the reader's sidebar applied to
the page: every section owns a hue from `design.HomeHues`, which is `sourceHues` in declaration order
plus the first generated one — the eighth section wears exactly what the eighth feed would.

**Each section's eyebrow carries a status.** *three rungs of four*, *the good part needs your own
key*, *the profile you can argue with is next*. A homepage that lists only what works is a homepage
somebody discovers the gaps in afterwards.

Covered by `e2e/home.spec.mjs`: every section renders, every screenshot loads and has alternative
text, `j`/`k` move and the rail follows, `?` and Escape work, a rail row navigates, and nothing
